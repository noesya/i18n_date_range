...
RSpec.describe I18nDateRange do
  it "Pas de date de fin" do
    ...
    expect(I18nDateRangeGenerator.new(date_from).to_s).to eq("8 septembre 2023")
    expect(I18nDateRangeGenerator.new(date_from, format: :short).to_s).to eq("8 septembre 2023")
    expect(I18nDateRangeGenerator.new(date_from, format: :long).to_s).to eq("Vendredi 8 septembre 2023")
  ...
  it "Jour différent" do
    ...
    expect(I18nDateRangeGenerator.new(date_from, date_to, layout: :two_lines).to_s).to eq("8<br>9 septembre 2023")
    expect(I18nDateRangeGenerator.new(date_from, date_to, format: :long, layout: :two_lines).to_s).to eq("Vendredi 8<br>samedi 9 septembre 2023")
  end
